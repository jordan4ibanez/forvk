...
contains
  ...
  function compile_glsl_shaders(logical_device, shader_file_name) result(shader_module)
    ...
    !? This is where we mishmash ShaderC and Vulkan SDK so it's a bit awkard with debug output lol.
    !? I simply decided to make it all output as module ShaderC. (Shader Compiler)
